...
- The oldest supported Python version is specified in [doc/dependencies.md](/doc/dependencies.md).
  ...
  The CI linter job also checks this, but [possibly not in all cases](https://github.com/hypercoin/hypercoin/pull/14884#discussion_r239585126).
...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the HypercoinTestFramework, place overrides for the
...
- Use `platform.system()` for detecting the running operating system and `os.name` to
  ...
  methods in the `HypercoinTestFramework` class, which can be used to skip a whole test
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x50=1250 HRC) in its wallet. Using them is much more
...
with the hypercoind(s) being tested in a callback-based event loop; the other
...
The [`TestShell`](test-shell.md) class exposes the HypercoinTestFramework
...
Test objects for interacting with a hypercoind node over the p2p interface.

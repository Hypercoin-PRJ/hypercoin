...
# The block reward of coinbaseoutput.nValue (50) HRC/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 HRC/block.
...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"hypercoin-cli -h\" for help or \"hypercoin-cli listwallets\" to see "
...
class TestHypercoinCli(HypercoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `hypercoin-cli getblockchaininfo`")
        ...
        # Prevent hypercoin-cli from using existing rpcport in conf
        ...
        self.log.info("Test -getinfo and hypercoin-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and hypercoin-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (HRC/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (HRC/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 HRC) to the default wallet.
            ...
            # Test hypercoin-cli -generate with -rpcwallet in multiwallet mode.
...
if __name__ == '__main__':
    TestHypercoinCli(__file__).main()

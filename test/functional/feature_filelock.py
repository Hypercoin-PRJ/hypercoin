...
"""Check that it's not possible to start a second hypercoind instance using the same datadir or wallet."""
import random
import string

from test_framework.test_framework import HypercoinTestFramework
from test_framework.test_node import (
    HYPERCOIN_PID_FILENAME_DEFAULT,
    ErrorMatch,
)

class FilelockTest(HypercoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second hypercoind instance using the same datadir")
        ...
        self.log.info("Check that we can't start a second hypercoind instance using the same blocksdir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second hypercoind using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second hypercoind instance shutdown
        pid_file = datadir / HYPERCOIN_PID_FILENAME_DEFAULT
        ...
        if self.is_wallet_compiled():
            ...
            self.log.info("Check that we can't start a second hypercoind instance using the same wallet")

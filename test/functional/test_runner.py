...
def main():
    ...
    tmpdir = "%s/hypercoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/hypercoin/hypercoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/hypercoin/hypercoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_utils and enable_hypercoind):
        print("No functional tests to run. Wallet, utils, and hypercoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if hypercoind is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "hypercoind"]) is not None:
    ...
    #Set env vars
    if "HYPERCOIND" not in os.environ:
        os.environ["HYPERCOIND"] = build_dir + '/src/hypercoind' + exeext
        os.environ["HYPERCOINCLI"] = build_dir + '/src/hypercoin-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie hypercoinds, we can apply a

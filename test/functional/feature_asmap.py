...
Verify node behaviour and debug log when launching hypercoind with different
...
class AsmapTest(HypercoinTestFramework):
    ...
    def test_embedded_asmap(self):
        if self.is_embedded_asmap_compiled():
            self.log.info('Test hypercoind -asmap (using embedded map data)')
            for arg in ['-asmap', '-asmap=1']:
                self.stop_node(0)
                with self.node.assert_debug_log(["Opened asmap data", "from embedded byte array"]):
                    self.start_node(0, [arg])
        else:
            self.log.info('Test hypercoind -asmap (compiled without embedded map data)')
            for arg in ['-asmap', '-asmap=1']:
                self.stop_node(0)
    ...
    def test_asmap_with_missing_file(self):
        self.log.info('Test hypercoind -asmap with missing map file')
        self.stop_node(0)

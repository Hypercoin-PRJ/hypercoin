...
"""Test the hypercoin wrapper tool."""
from test_framework.test_framework import (
    HypercoinTestFramework,
    SkipTest,
)
...
class ToolHypercoinTest(HypercoinTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `hypercoin node -version` is
        # run on windows, python doesn't capture output from the child
        # `hypercoind` and `hypercoin-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/hypercoin/hypercoin/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through hypercoin wrapper command with specified arguments."""
        # Manually construct the `hypercoin node` command, similar to Binaries::node_argv()
        hypercoin_cmd = node.binaries.valgrind_cmd + [node.binaries.paths.hypercoin_bin]
        node.args = hypercoin_cmd + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure hypercoin node command invokes hypercoind by default")
        self.test_args([], [], expect_exe="hypercoind")

        self.log.info("Ensure hypercoin -M invokes hypercoind")
        self.test_args(["-M"], [], expect_exe="hypercoind")

        self.log.info("Ensure hypercoin -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure hypercoin -m invokes hypercoin-node")
            self.test_args(["-m"], [], expect_exe="hypercoin-node")

            self.log.info("Ensure hypercoin -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="hypercoin-node")

            self.log.info("Ensure hypercoin accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="hypercoin-node")

            self.log.info("Ensure hypercoin recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="hypercoin-node")
...
if __name__ == '__main__':
    ToolHypercoinTest(__file__).main()

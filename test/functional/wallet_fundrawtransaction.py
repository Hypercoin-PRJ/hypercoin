...
class RawTransactionsTest(HypercoinTestFramework):
    ...
    def test_invalid_change_address(self):
        ...
        assert_raises_rpc_error(-5, "Change address must be a valid hypercoin address", self.nodes[2].fundrawtransaction, rawtx, changeAddress='foobar')
    ...
    def test_option_feerate(self):
        self.log.info("Test fundrawtxn with explicit fee rates (fee_rate sat/vB and feeRate HRC/kvB)")
        node = self.nodes[3]
        ...
        hrc_kvb_to_sat_vb = 100000  # (1e5)
        result1 = node.fundrawtransaction(rawtx, fee_rate=str(2 * hrc_kvb_to_sat_vb * self.min_relay_tx_fee))
        result2 = node.fundrawtransaction(rawtx, feeRate=2 * self.min_relay_tx_fee)
        result3 = node.fundrawtransaction(rawtx, fee_rate=10 * hrc_kvb_to_sat_vb * self.min_relay_tx_fee)
        ...
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (sat/vB) and feeRate (HRC/kvB)",
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        # Test subtract fee from outputs with feeRate (HRC/kvB)
        ...
        result = [self.nodes[3].fundrawtransaction(rawtx, fee_rate=self.fee_rate_sats_per_vb),  # uses self.min_relay_tx_fee (set by fee_rate in sat/vB)
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[], fee_rate=self.fee_rate_sats_per_vb),  # empty subtraction list
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[0], fee_rate=self.fee_rate_sats_per_vb),  # uses self.min_relay_tx_fee (set by fee_rate in sat/vB)
            self.nodes[3].fundrawtransaction(rawtx, fee_rate=2 * hrc_kvb_to_sat_vb * self.min_relay_tx_fee),
            self.nodes[3].fundrawtransaction(rawtx, fee_rate=2 * hrc_kvb_to_sat_vb * self.min_relay_tx_fee, subtractFeeFromOutputs=[0]),]
        dec_tx = [self.nodes[3].decoderawtransaction(tx_['hex']) for tx_ in result]
        output = [d['vout'][1 - r['changepos']]['value'] for d, r in zip(dec_tx, result)]
        change = [d['vout'][r['changepos']]['value'] for d, r in zip(dec_tx, result)]
    ...
    def test_transaction_too_large(self):
        ...
        # Make 1500 0.1 HRC outputs. The amount that we target for funding is in
    ...
    def test_add_inputs_default_value(self):
        ...
        # Create and fund the wallet with 5 HRC
        ...
        # Fund wallet with 2 outputs, 5 HRC each.
    ...
    def test_preset_inputs_selection(self):
        ...
        # Create and fund the wallet with 4 UTXO of 5 HRC each (20 HRC total)
        ...
        # Attempt to send 29 HRC from a wallet that only has 20 HRC. The wallet should exclude
        # the preset inputs from the pool of available coins, realize that there is not enough
        # money to fund the 29 HRC payment, and fail with "Insufficient funds".
        #
        # Even with SFFO, the wallet can only afford to send 20 HRC.
        # If the wallet does not properly exclude preset inputs from the pool of available coins
        # prior to coin selection, it may create a transaction that does not fund the full payment
        # amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
        # between the original target and the wrongly counted inputs (in this case 9 HRC)
        # so that the recipient's amount is no longer equal to the user's selected target of 29 HRC.
    ...
    def test_22670(self):
        ...
        # that this bug is not detected because the transaction fee must be at least 0.01 HRC (the minimum change value).
        # Otherwise the targeted minimum change value will be enough to cover the transaction fees that were not
        # being accounted for. So the minimum relay fee is set to 0.1 HRC/kvB in this test.
    ...
    def test_feerate_rounding(self):
        ...
        # fail with insufficient funds rather than hypercoind asserting.
    ...
    def test_input_confs_control(self):
        ...
        # Create a replacement tx to 'final_tx1' that has 1 HRC target instead of 0.1.
        ...
        # Now fund 'raw_tx2' to fulfill the total target (1 HRC) by using all the wallet unconfirmed outputs.
        # As it was created with the first unconfirmed output, 'raw_tx2' only has 0.1 HRC covered (need to fund 0.9 HRC more).
    ...
    def test_cannot_cover_fees(self):
        ...
        # Set up wallet with 2 utxos: 0.3 HRC and 0.15 HRC
        ...
        self.log.info("Attempt to send 0.45 HRC without SFFO")
        ...
        self.log.info("Send 0.45 HRC with SFFO")
        wallet.fundrawtransaction(rawtx, options={"subtractFeeFromOutputs":[0]})

        self.log.info("Attempt to send 0.45 HRC by restricting coin selection with minconf=6")
        ...
        self.log.info("Attempt to send 0.45 HRC preselecting 0.15 HRC utxo")
        rawtx = wallet.createrawtransaction(inputs=[{"txid": txid2, "vout": vout2}], outputs=[{default_wallet.getnewaddress(): 0.45}])
        assert_raises_rpc_error(-4, amount_with_fee_err_msg.format("0.00000042"), wallet.fundrawtransaction, rawtx, options={"fee_rate":1})

        self.log.info("Send 0.45 HRC preselecting 0.15 HRC utxo with SFFO")
        wallet.fundrawtransaction(hexstring=rawtx, options={"subtractFeeFromOutputs":[0]})

        self.log.info("Attempt to send 0.15 HRC using only the 0.15 HRC preselected utxo")
        ...
        self.log.info("Send 0.15 HRC using only the 0.15 HRC preselected utxo with SFFO")

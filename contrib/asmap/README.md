...
be used in Hypercoin Core with the `-asmap` option.
...
to binary before being used with Hypercoin Core.
...
the `hypercoin-cli getnodeaddresses` command.
...
```
hypercoin-cli getnodeaddresses 0 > addrs.json
```

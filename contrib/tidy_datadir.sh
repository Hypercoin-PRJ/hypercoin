#!/bin/bash
# Copyright (c) 2013 The Hypercoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Hypercoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Hypercoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Hypercoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Hypercoin 0.7 datadir."
    ;;
  3)
    echo "Detected Hypercoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Hypercoin 0.8 datadir."
    ;;
esac

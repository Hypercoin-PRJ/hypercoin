...
###   hypercoincore.org and hypercoin.org and compares them.
...
WORKINGDIR="/tmp/hypercoin_verify_binaries"
TMPFILE="hashes.tmp"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'hypercoin-',
...
if [ $? -ne 0 ]; then
   echo "hypercoin.org failed to provide signature file, but hypercoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "hypercoin.org and hypercoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Hypercoin Core binary release signing key installed?"
   fi

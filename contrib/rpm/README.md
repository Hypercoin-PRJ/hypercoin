...
The RPM spec file provided here is for Hypercoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://hypercoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Hypercoin git repository](https://github.com/hypercoin/hypercoin) but are not part of
...
Currently only `Source10` is used, for the example `hypercoin.conf` file.
...
in the hypercoin git repository. At present only an SVG version of the Hypercoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the hypercoin
...
needed when 0.12.1 is released, a proper fix is already in the Hypercoin git
master branch.
...
idea to build Hypercoin Core against a static version of that library compiled
...
The `hypercoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `hypercoin-server` package. At this point, most RPM based
...
The `hypercoin-server` package should have SELinux support. How to properly do
...
Hypercoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/hypercoin/). He has been
packaging Hypercoin for Fedora at least since 2012.
...
4. I renamed the `hypercoin` package that contains the Qt GUI to `hypercoin-core` as
that appears to be how the general population refers to it, in contrast to
`hypercoin-xt` or `hypercoin-classic`. I wanted to make sure the general population

# Bootstrappable Hypercoin Core Builds

This directory contains the files necessary to perform bootstrappable Hypercoin
Core builds.
...
To build Hypercoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Hypercoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/hypercoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/hypercoin-core/guix.sigs).

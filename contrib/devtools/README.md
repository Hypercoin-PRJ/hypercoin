...
gen-hypercoin-conf.sh
===================

Generates a hypercoin.conf file in `share/examples/` by parsing the output from `hypercoind --help`. This script is run during the
release process to include a hypercoin.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-hypercoin-conf.sh
```

...
Tracing (USDT) support for Hypercoin Core on Linux using. For more information on
USDT support in Hypercoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `hypercoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `hypercoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Hypercoin Core USDT documentation] on how to list available tracepoints in your
`hypercoind` binary.

[Hypercoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof hypercoind)
```
...
Hypercoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof hypercoind)
```
...
In a different terminal, starting Hypercoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/hypercoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof hypercoind)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof hypercoind)
```

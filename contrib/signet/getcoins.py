...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing hypercoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='hypercoin-cli', help='hypercoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Hypercoin address to which the faucet should send')
...
parser.add_argument('hypercoin_cli_args', nargs='*', help='Arguments to pass on to hypercoin-cli (default: -signet)')

args = parser.parse_args()

if args.hypercoin_cli_args == []:
    args.hypercoin_cli_args = ['-signet']


def hypercoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.hypercoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = hypercoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = hypercoin_cli(['getnewaddress', 'faucet', 'bech32'])

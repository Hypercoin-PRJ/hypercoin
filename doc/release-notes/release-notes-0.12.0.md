Hypercoin Core version 0.12.0 is now available from:

  <https://hypercoin.org/bin/hypercoin-core-0.12.0/>
...
pre-0.12 versions of Hypercoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Hypercoin Core version 0.11 or
earlier.
...
ECDSA signatures inside Hypercoin transactions now use validation using
[libsecp256k1](https://github.com/hypercoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/hypercoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Hypercoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Hypercoin Core was to raise the default minimum
relay fee.

Hypercoin Core 0.12 will have a strict maximum size on the mempool. The
...
Hypercoin Core 0.12 also introduces new default policy limits on the length and
...
Hypercoin Core 0.12 nodes. Hypercoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/hypercoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Hypercoin Core 0.12 does not yet have support for
...
This allows running hypercoind without having to do any manual configuration.
...
Hypercoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Hypercoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Hypercoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Hypercoin Core 0.13, and it is at this time undecided whether the
...
Hypercoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Hypercoin Core automatically creates a hidden service to listen on, without
manual configuration. Hypercoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Hypercoin Core is listening, and
...
Hypercoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Hypercoin
Core to use floating fees. By default, Hypercoin Core will use floating
fees.
...
(default: `0.0002` HRC/kB).

At all times, Hypercoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) HRC.
Furthermore, Hypercoin Core will never create transactions paying less than
...
`hypercoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/hypercoin/hypercoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/hypercoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/hypercoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /hypercoinrpc>
        ...
        # optional bypass hypercoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< hypercoinrpc:<password>
    </Location>
...
Restarting hypercoind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Hypercoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new hypercoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include hypercoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix hypercoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of hypercoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override HYPERCOIND and HYPERCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [hypercoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add HYPERCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libhypercoinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "HRC" (Ross Nicoll)

...
Hypercoin Core version v30.0 is now available from:

  <https://hypercoincore.org/bin/hypercoin-core-30.0/>
...
installer (on Windows) or just copy over `/Applications/Hypercoin-Qt` (on macOS)
...
Hypercoin Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. Hypercoin
...
New `hypercoin` command
---------------------

- A new `hypercoin` command line tool has been added to make features more discoverable
  and convenient to use. The `hypercoin` tool just calls other executables and does not
  implement any functionality on its own. Specifically `hypercoin node` is a synonym for
  `hypercoind`, `hypercoin gui` is a synonym for `hypercoin-qt`, and `hypercoin rpc` is a synonym
  for `hypercoin-cli -named`. Other commands and options can be listed with `hypercoin help`.
  The new `hypercoin` command is an alternative to calling other commands directly, but it
...
- The new `hypercoin` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `hypercoin -m node -ipcbind=unix` it will
  ...
  (`hypercoin-node` instead of `hypercoind`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/hypercoin/hypercoin/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/hypercoin/hypercoin/blob/master/doc/multiprocess.md)),
...
- The `test_hypercoin` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `hypercoin` command
  as `hypercoin test`. The `libexec/` directory also contains new `hypercoin-node` and
  `hypercoin-gui` binaries which support IPC features and are called through the
  `hypercoin` tool. In source builds only, `test_hypercoin-qt`, `bench_hypercoin`, and
  `hypercoin-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `hypercoin` command. See `hypercoin help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of Hypercoin Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in Hypercoin Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the hypercoin-wallet tool,
...
[Transifex](https://explore.transifex.com/hypercoin/hypercoin/).

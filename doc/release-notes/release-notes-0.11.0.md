Hypercoin Core version 0.11.0 is now available from:

  <https://hypercoin.org/bin/hypercoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer HRC/kB fee to be
...
For example, add the following to `hypercoin.conf`:
...
related to the blockchain in the hypercoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Hypercoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Hypercoin Core will 
...
For an example script see [hypercoin-submittx](https://github.com/laanwj/hypercoin-submittx).
...
- #5334 `cf87536` libhypercoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new hypercoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only HypercoinAmountField
- #5753 `0f44672` Add hypercoin logo to about screen
...
- #5809 `46bfbe7` Add hypercoin-cli man page
...
- #5849 `41259ca` contrib/init/hypercoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Hypercoin-Qt.app to Hypercoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for hypercoin-tx)

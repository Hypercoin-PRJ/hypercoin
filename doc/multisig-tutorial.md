...
for ((n=1;n<=3;n++))
do
 ./build/bin/hypercoin rpc -signet createwallet "participant_${n}"
done
```
...
Note that previously at least two descriptors were usually used, one for external derivation paths and one for internal ones. Since https://github.com/hypercoin/hypercoin/pull/22838 this redundancy has been eliminated by a multipath descriptor with <code><0;1></code> at the [BIP-44](https://github.com/hypercoin/bips/blob/master/bip-0044.mediawiki#change) change level expanding to external and internal descriptors when imported.
...
for ((n=1;n<=3;n++))
do
 xpubs["xpub_${n}"]=$(./build/bin/hypercoin rpc -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))' | sed 's /0/\* /<0;1>/* ')
done
```
...
checksum=$(./build/bin/hypercoin rpc -signet getdescriptorinfo $desc | jq -r '.checksum')
...
Documentation for these and other parameters can be found by typing `./build/bin/hypercoin rpc -signet help importdescriptors`.
...
```bash
./build/bin/hypercoin rpc -signet createwallet "multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/hypercoin rpc -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/hypercoin rpc -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/hypercoin rpc -signet loadwallet "participant_${n}"; done
./build/bin/hypercoin rpc -signet loadwallet "multisig_wallet_01"
```
...
```bash
receiving_address=$(./build/bin/hypercoin rpc -signet -rpcwallet="multisig_wallet_01" getnewaddress)
...
```bash
./build/bin/hypercoin rpc -signet -rpcwallet="multisig_wallet_01" getbalances
```
...
```bash
balance=$(./build/bin/hypercoin rpc -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/hypercoin rpc -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/hypercoin rpc -signet -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./build/bin/hypercoin rpc -signet decodepsbt $funded_psbt

./build/bin/hypercoin rpc -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/hypercoin rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/hypercoin rpc -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/hypercoin rpc -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/hypercoin rpc -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/hypercoin rpc -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/hypercoin rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/hypercoin rpc -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/hypercoin rpc -signet finalizepsbt $psbt_2 | jq -r '.hex')

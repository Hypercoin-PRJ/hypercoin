...
Hypercoin Core has an internal benchmarking framework, with benchmarks
...
For benchmarking, you only need to compile `bench_hypercoin`.  The bench runner
warns if you configure with `-DCMAKE_BUILD_TYPE=Debug`, but consider if building without
it will impact the benchmark(s) you are interested in by unlatching log printers
and lock analysis.

    cmake -B build -DBUILD_BENCH=ON
    cmake --build build -t bench_hypercoin

After compiling hypercoin-core, the benchmarks can be run with:

    build/bin/bench_hypercoin
...
To monitor Hypercoin Core performance more in depth (like reindex or IBD): https://github.com/hypercoin-dev-tools/benchcoin

# CJDNS support in Hypercoin Core

It is possible to run Hypercoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Hypercoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Hypercoin network and individual nodes.
...
before it will work with your Hypercoin Core node. This is described in steps
...
## Run Hypercoin Core with CJDNS

Once you are connected to the CJDNS network, the following Hypercoin Core
...
When enabled, this option tells Hypercoin Core that it is running in an
...
IPv6 local network. This helps Hypercoin Core perform better address management:
...
CJDNS support was added to Hypercoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `hypercoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/hypercoin/hypercoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Hypercoin Core

There are several ways to see your CJDNS address in Hypercoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `hypercoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `hypercoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `hypercoin-cli help getnodeaddresses` for details.

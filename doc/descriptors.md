...
Since Hypercoin Core v0.17, there is support for Output Descriptors. This is a
...
Hypercoin Core v24 extended `wsh()` output descriptor with [Miniscript](https://hypercoin.sipa.be/miniscript/) support (initially watch-only). Signing support for Miniscript descriptors was added in v25. And since v26 Miniscript expressions can now be used in Taproot descriptors.
...
- [Miniscript](https://hypercoin.sipa.be/miniscript/) expressions in `wsh` (P2WSH) and `tr` (P2TR) functions.
...
the entire path from the master down. [BIP174](https://github.com/hypercoin/bips/blob/master/bip-0174.mediawiki) standardizes this by

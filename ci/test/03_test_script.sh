...
(
  ...
  # See https://github.com/hypercoin/hypercoin/pull/28359#issuecomment-1698694748
...
HYPERCOIN_CONFIG_ALL="-DBUILD_BENCH=ON -DBUILD_FUZZ_BINARY=ON"
if [ -z "$NO_DEPENDS" ]; then
  HYPERCOIN_CONFIG_ALL="${HYPERCOIN_CONFIG_ALL} -DCMAKE_TOOLCHAIN_FILE=$DEPENDS_DIR/$HOST/toolchain.cmake"
fi
if [ -z "$NO_WERROR" ]; then
  HYPERCOIN_CONFIG_ALL="${HYPERCOIN_CONFIG_ALL} -DWERROR=ON"
fi
...
bash -c "cmake -S $BASE_ROOT_DIR -B ${BASE_BUILD_DIR} $HYPERCOIN_CONFIG_ALL $HYPERCOIN_CONFIG" || (
  cd "${BASE_BUILD_DIR}"
...
if [ "${RUN_TIDY}" = "true" ]; then
  ...
  if ! ( run-clang-tidy-"${TIDY_LLVM_V}" -quiet -load="/tidy-build/libhypercoin-tidy.so" "${MAKEJOBS}" | tee tmp.tidy-out.txt ); then

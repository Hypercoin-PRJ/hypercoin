...
if [ "$RUN_FUZZ_TESTS" = "true" ]; then
  ...
  if [ ! -d "$DIR_FUZZ_IN" ]; then
    ${CI_RETRY_EXE} git clone --depth=1 https://github.com/hypercoin-core/qa-assets "${DIR_QA_ASSETS}"
  fi
  (
    cd "${DIR_QA_ASSETS}"
  ...
  if [ ! -d "$DIR_UNIT_TEST_DATA" ]; then
    ...
    ${CI_RETRY_EXE} curl --location --fail https://github.com/hypercoin-core/qa-assets/raw/main/unit_test_data/script_assets_test.json -o "${DIR_UNIT_TEST_DATA}/script_assets_test.json"
  fi
fi

# Make sure default datadir does not exist and is never read by creating a dummy file
if [ "$CI_OS_NAME" == "macos" ]; then
  echo > "${HOME}/Library/Application Support/Hypercoin"
else
  echo > "${HOME}/.hypercoin"
fi
...
HYPERCOIN_CONFIG_ALL="-DCMAKE_COMPILE_WARNING_AS_ERROR=ON -DBUILD_BENCH=ON -DBUILD_FUZZ_BINARY=ON"
if [ -z "$NO_DEPENDS" ]; then
  HYPERCOIN_CONFIG_ALL="${HYPERCOIN_CONFIG_ALL} -DCMAKE_TOOLCHAIN_FILE=$DEPENDS_DIR/$HOST/toolchain.cmake"
fi
...
HYPERCOIN_CONFIG_ALL="$HYPERCOIN_CONFIG_ALL -DCMAKE_INSTALL_PREFIX=$BASE_OUTDIR -Werror=dev"

if [[ "${RUN_IWYU}" == true || "${RUN_TIDY}" == true ]]; then
  HYPERCOIN_CONFIG_ALL="$HYPERCOIN_CONFIG_ALL -DCMAKE_EXPORT_COMPILE_COMMANDS=ON"
fi

eval "CMAKE_ARGS=($HYPERCOIN_CONFIG_ALL $HYPERCOIN_CONFIG)"
...
if [[ "$CI_OS_NAME" == "macos" && "${GOAL}" = "install deploy" ]]; then
  unzip "${BASE_BUILD_DIR}/hypercoin-macos-app.zip" -d "${BASE_BUILD_DIR}/deploy"
  if ! ( codesign --verify "${BASE_BUILD_DIR}/deploy/Hypercoin-Qt.app" ); then
...
if [ "${RUN_TIDY}" = "true" ]; then
  cmake -B /tidy-build -DLLVM_DIR=/usr/lib/llvm-"${TIDY_LLVM_V}"/cmake -DCMAKE_BUILD_TYPE=Release -S "${BASE_ROOT_DIR}"/contrib/devtools/hypercoin-tidy
  cmake --build /tidy-build "$MAKEJOBS"
  cmake --build /tidy-build --target hypercoin-tidy-tests "$MAKEJOBS"
  ...
  if ! ( run-clang-tidy-"${TIDY_LLVM_V}" -config-file="${BASE_ROOT_DIR}/src/.clang-tidy" -quiet -load="/tidy-build/libhypercoin-tidy.so" "${MAKEJOBS}" | tee tmp.tidy-out.txt ); then
...
if [[ "${RUN_IWYU}" == true ]]; then
  ...
  run_iwyu() {
    ...
    python3 "/include-what-you-use/iwyu_tool.py" \
             ...
             -- -Xiwyu --cxx17ns -Xiwyu --mapping_file="${BASE_ROOT_DIR}/contrib/devtools/iwyu/hypercoin.core.imp" \

...
def check_manifests(ci_type):
    ...
    manifest_path = release_dir / "hypercoind.manifest"
    cmd_hypercoind_manifest = [
        "mt.exe",
        "-nologo",
        f"-inputresource:{release_dir / 'hypercoind.exe'}",
        f"-out:{manifest_path}",
    ]
    run(cmd_hypercoind_manifest)
    print(manifest_path.read_text())

    skips = {  # Skip as they currently do not have manifests
        "fuzz.exe",
        "bench_hypercoin.exe",
        "test_hypercoin-qt.exe",
        "hypercoin-chainstate.exe",
    }
...
def prepare_tests(ci_type):
    ...
    if ci_type == "standard":
        ...
        clone_cmd = [
            ...
            "clone",
            "--depth=1",
            "https://github.com/hypercoin-core/qa-assets",
            repo_dir,
        ]
        run(clone_cmd)
...
def run_tests(ci_type):
    ...
    if ci_type == "standard":
        ...
        test_envs = {
            "HYPERCOIN_BIN": "hypercoin.exe",
            "HYPERCOIND": "hypercoind.exe",
            "HYPERCOINCLI": "hypercoin-cli.exe",
            "HYPERCOIN_BENCH": "bench_hypercoin.exe",
            "HYPERCOINTX": "hypercoin-tx.exe",
            "HYPERCOINUTIL": "hypercoin-util.exe",
            "HYPERCOINWALLET": "hypercoin-wallet.exe",
            "HYPERCOINCHAINSTATE": "hypercoin-chainstate.exe",
        }
        ...
        os.environ["HYPERCOINFUZZ"] = str(release_bin / "fuzz.exe")
        fuzz_cmd = [
            sys.executable,

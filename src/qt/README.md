This directory contains the source code for the Hypercoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Hypercoin UI.
...
- ClientModel is used by the main application `hypercoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a hypercoin:... link from a browser).
...
* UI elements like HypercoinAmountField, which inherit from QWidget.
* `hypercoinstrings.cpp`: automatically generated
* `hypercoinunits.(h/cpp)`: HRC / mHRC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/hypercoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `hypercoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `hypercoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `hypercoin-qt`
9. You're all set! Start developing, building, and debugging the Hypercoin Core GUI

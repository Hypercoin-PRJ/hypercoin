...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<HypercoinUnit>("HypercoinUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. hypercoin_de.qm (shortcut "de" needs to be defined in hypercoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. hypercoin_de_DE.qm (shortcut "de_DE" needs to be defined in hypercoin.qrc)
...
static const char* qt_argv = "hypercoin-qt";

HypercoinApplication::HypercoinApplication()
...
void HypercoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the HypercoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", HypercoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void HypercoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool HypercoinApplication::createOptionsModel(bool resetSettings)
{
...
void HypercoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new HypercoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &HypercoinGUI::quitRequested, this, &HypercoinApplication::requestShutdown);
...
void HypercoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void HypercoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool HypercoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void HypercoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &HypercoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &HypercoinApplication::handleRunawayException);
    connect(this, &HypercoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &HypercoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void HypercoinApplication::parameterSetup()
{
...
void HypercoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void HypercoinApplication::requestInitialize()
{
...
void HypercoinApplication::requestShutdown()
{
...
void HypercoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    // hypercoin: URIs or payment requests:
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &HypercoinGUI::handlePaymentRequest);
        connect(window, &HypercoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void HypercoinApplication::handleRunawayException(const QString &message)
{
...
void HypercoinApplication::handleNonFatalException(const QString& message)
{
...
WId HypercoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool HypercoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", HypercoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(hypercoin);
    Q_INIT_RESOURCE(hypercoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(HYPERCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see hypercoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see hypercoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse hypercoin.conf, determine network, switch to network specific
    ...
    // hypercoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {

...
/** Call node.updateRwSetting() with Hypercoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Hypercoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/hypercoin/hypercoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because hypercoin 22.x will not try to read these.
...
//! Convert enabled/size values to hypercoin -prune setting.
...
//! Get pruning enabled value to show in GUI from hypercoin -prune setting.
...
//! Get pruning size value to show in GUI from hypercoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayHypercoinUnit")) {
        settings.setValue("DisplayHypercoinUnit", QVariant::fromValue(HypercoinUnit::HRC));
    }
    QVariant unit = settings.value("DisplayHypercoinUnit");
    if (unit.canConvert<HypercoinUnit>()) {
        m_display_hypercoin_unit = unit.value<HypercoinUnit>();
    } else {
        m_display_hypercoin_unit = HypercoinUnit::HRC;
        settings.setValue("DisplayHypercoinUnit", QVariant::fromValue(m_display_hypercoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if hypercoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_hypercoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<HypercoinUnit>() == m_display_hypercoin_unit) return;
    m_display_hypercoin_unit = new_unit.value<HypercoinUnit>();
    QSettings settings;
    settings.setValue("DisplayHypercoinUnit", QVariant::fromValue(m_display_hypercoin_unit));
    Q_EMIT displayUnitChanged(m_display_hypercoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/hypercoin/hypercoin/pull/8273
    ...
    // (https://github.com/hypercoin-core/gui/issues/567).

...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    BOOST_CHECK_THROW(process->bind(datadir, "test_hypercoin", invalid_bind), std::invalid_argument);
    BOOST_CHECK_THROW(process->connect(datadir, "test_hypercoin", invalid_bind), std::invalid_argument);

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        int serve_fd = process->bind(datadir, "test_hypercoin", address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        int connect_fd{process->connect(datadir, "test_hypercoin", address)};
    ...
    //   Address 'unix' path '"/tmp/test_common_Hypercoin Core/ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff/test_hypercoin.sock"' exceeded maximum socket path length
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath("hypercoin_sock0_XXXXXX")),
        strprintf("unix:%s", TempPath("hypercoin_sock1_XXXXXX")),
    };

...
configure_hypercoin_core() {
  local cmake_arg
  local cmake_args=()

  if [[ -n "${HYPERCOIN_CORE_CMAKE_ARGS:-}" ]]; then
    while IFS= read -r cmake_arg; do
      [[ -n "${cmake_arg}" ]] || continue
      cmake_args+=("${cmake_arg}")
    done <<< "${HYPERCOIN_CORE_CMAKE_ARGS}"
  fi
...
run_ipc_unit_tests() {
  ...
  for _ in $(seq 1 "${runs}"); do
    build/bin/test_hypercoin --run_test=ipc_tests,miner_tests --catch_system_error=no --log_level=nothing --report_level=no
  done
}

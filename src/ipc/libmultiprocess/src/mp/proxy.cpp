...
thread_local ThreadContext g_thread_context; // NOLINT(hypercoin-nontrivial-threadlocal)
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking hypercoin code so they have to run
    ...
    // functions run, so client objects are fully disconnected before hypercoin
    // code in the destructors are run. This way if the hypercoin code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In hypercoin core, running these callbacks in the right order is

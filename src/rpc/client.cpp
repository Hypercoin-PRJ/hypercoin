...
//! Specify whether parameter should be parsed by hypercoin-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make hypercoin-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `hypercoin-cli` attempt to parse the
 ...
 * `hypercoin-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make hypercoin-cli treat them as positional parameters when `-named` is used.
 * This prevents `hypercoin-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so hypercoin-cli
 ...
 *
 * For example, the command `hypercoin-cli -named createwallet "my=wallet"`,

...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), HYPERCOIN_CONF_FILENAME));
    if (mode == HMM_HYPERCOIND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), HYPERCOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/hypercoin/hypercoin>";
    const std::string URL_WEBSITE = "<https://hypercoincore.org>";
...
/** Sanity checks
 *  Ensure that Hypercoin is running in a usable environment with all
...
{
    // Make sure only a single Hypercoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", HYPERCOIN_CONF_FILENAME)).string());

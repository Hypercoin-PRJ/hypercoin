...
static void SetupHypercoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " hypercoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The hypercoin-tx tool is used for creating and modifying hypercoin transactions.\n\n"
                "hypercoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded hypercoin transaction, or with \"-create [commands]\" to create a hex-encoded hypercoin transaction.\n"
                "\n"
                "Usage: hypercoin-tx [options] <hex-tx> [commands]\n"
                "or:    hypercoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded hypercoin transaction

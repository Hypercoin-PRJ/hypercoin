...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        ...
        // hypercoin.conf file just ignores the other file.)
        const fs::path orig_datadir_path{args.GetDataDirBase()};
        const fs::path orig_config_path{AbsPathForConfigVal(args, args.GetPathArg("-conf", HYPERCOIN_CONF_FILENAME), /*net_specific=*/false)};
        ...
        if (!fs::exists(base_path)) {
            ...
            // in https://github.com/hypercoin/hypercoin/issues/16220 about ways to
        ...
        // Show an error or warn/log if there is a hypercoin.conf file in the
        // datadir that is being ignored.
        const fs::path base_config_path = base_path / HYPERCOIN_CONF_FILENAME;
        if (fs::exists(base_config_path)) {
            if (orig_config_path.empty()) {
                LogInfo(
                    ...
                    fs::quoted(HYPERCOIN_CONF_FILENAME));

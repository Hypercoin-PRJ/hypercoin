...
Options:
  -m, --multiprocess     Run multiprocess binaries hypercoin-node, hypercoin-gui.
  -M, --monolithic       Run monolithic binaries hypercoind, hypercoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'hypercoin-qt [ARGS]' or 'hypercoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'hypercoind [ARGS]' or 'hypercoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'hypercoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'hypercoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'hypercoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_hypercoin [ARGS]'.
  chainstate [ARGS] Run hypercoin kernel chainstate util, equivalent to running 'hypercoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_hypercoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_hypercoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "hypercoin-gui" : "hypercoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "hypercoin-node" : "hypercoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("hypercoin-cli");
            // Since "hypercoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("hypercoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("hypercoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_hypercoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("hypercoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_hypercoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_hypercoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("hypercoin-util");
        } else {
...
//! Execute the specified hypercoind, hypercoin-qt or other command line in `args`
...
//!             be a relative path to a hypercoind, hypercoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/hypercoin ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/hypercoin/hypercoin/pull/31375#discussion_r1861814807)

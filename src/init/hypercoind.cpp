...
namespace init {
namespace {
const char* EXE_NAME = "hypercoind";

class HypercoindInit : public interfaces::Init
{
public:
    HypercoindInit(NodeContext& node) : m_node(node)
    {
        InitContext(m_node);
        m_node.init = this;
    }
...
{
    return std::make_unique<init::HypercoindInit>(node);
}
} // namespace interfaces
